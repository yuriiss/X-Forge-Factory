...
<sub>X-Forge v0.0.4 · автор Yurii S. ([@yuriiss](https://github.com/yuriiss)) · AGPL-3.0-or-later</sub>
...
```
npm install
npm run setup        # запише .env.local і перевірить ваш ключ
npm run build
npm start            # http://127.0.0.1:7777
```

`npm run dev` теж є — це сервер розробки: гаряче перезавантаження і повільніший перший
показ. Він для тих, хто змінює X-Forge, а не користується ним.

Щоб консоль поверталася після перезавантаження, `npm run service:install` створює
користувацький юніт systemd на Linux або агент launchd на macOS, який працює від вашого
імені, а не від root: консоль запускає CLI на цій машині і читає сховище у вашій домашній
теці, тож служба від іншого користувача не знайшла б ані того, ані іншого. У Windows вона
натомість друкує, який запис створити в Планувальнику завдань.
...
![Сток](docs/images/uk/stock.png)

Десять бібліотек за одним полем пошуку. Шість із них — це той самий ендпоінт ресурсів із
фільтром за типом контенту: фото, векторні зображення, ілюстрації, шаблони, PSD, мокапи, — а
інші чотири мають власні ендпоінти: відео, іконки, музика, звукові ефекти. Введіть запит,
натисніть Enter або `SEARCH`.

Сам сайт провайдера також перелічує 3D-моделі та шрифти. Тут їх немає, бо жоден фільтр, який
підтримує цей API, їх не вибирає: відповідь повертається нефільтрованою, а вкладка, що
непомітно показує фото з підписом «fonts», гірша за вкладку, якої просто немає.

### Отримання файлу

`DL` завантажує. Файл потрапляє **сюди** — його отримують за підписаним URL провайдера і
розміщують у сховищі поряд з усім, що створив X-Forge, з тим самим іменуванням і тією самою
нотаткою у markdown. Підпис поруч усе одно веде на сторінку елемента у провайдера — якщо
потрібна ліцензія чи автор, а не сам файл.

Клацання по плитці відкриває її в повний розмір; відео відтворюється прямо там. Звукові ефекти
відтворюються зі списку. Музика теж відтворюється, але перше натискання спершу завантажує
трек — бібліотека не повертає URL попереднього перегляду, лише URL для завантаження, тож
відтворення треку коштує того самого запиту, що й завантаження. На тарифі зі ста завантаженнями
на день це варто знати, перш ніж прослухати двадцять треків.

### Скільки це коштує

Сток **не** тарифікується в кредитах. На тарифі нижче Business ви отримуєте 100 завантажень на
день; на Business і вище — необмежено. Панелі внизу документують правила, який ендпоінт читає
